...
- 产出 AI / 算法主题的小红书纯图文笔记方案，包括标题、分镜、每页配文、画面描述、GPT-image2 Prompt 草案、正文、标签和复盘口径。
...
- 内容形态：纯图文 carousel。
- 默认画幅：小红书 3:4。
- 默认页数：根据算法步骤和难易程度动态决定；简单题可 3 到 4 页，常规题 4 到 6 页，复杂题拆上下篇；算法科普必须包含总结页。
- 视觉风格：软萌治愈 + 贴纸手账。
...
- 先用生活场景解释，再轻轻点出技术概念。
- 必须保留一个可被用户带走的一句话总结。
- 算法科普图文必须按机制步骤和难易程度动态定页数，并保留最后总结页或大体了解图；不得退回只有解释图、没有总结页的旧结构。
...
```powershell
rg "CP-SAT|软萌治愈|贴纸手账|小红书|AI / 算法|外部 LLM|去 AI 味|Image2" README.md AGENTS.md xiaohongshu-cute-knowledge-prd.md templates prompts drafts
```

## 后续优先事项

1. 用其他算法科普例子测试 `templates/algorithm-xhs-infographic-generator-template.md` 的泛化效果。
2. 每次生成前先根据算法步骤和难易程度确认页数，最后一页必须是总结图。
3. 对比不同生活例子的理解成本、阅读负担和生图稳定性。
