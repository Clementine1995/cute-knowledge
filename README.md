...
- 内容形态：纯图文 carousel。
- 默认画幅：小红书 3:4。
- 默认页数：能 2 页讲清楚就 2 页，常规题 3 到 5 页，复杂题拆上下篇。
- 视觉风格：软萌治愈 + 贴纸手账。
- 图片生成：由创作者使用 GPT-image2 自行生成，项目文档只负责提示词和画面方案。
- 文案立场：中性解释，少做强观点输出。
- 首篇样稿：CP-SAT，用“安排座位”解释约束求解，同时产出 2 页版和 4 页版。
...
1. 基于 CP-SAT 选题生成外部 LLM 专用提示词。
2. 等用户贴回外部 LLM 粗稿后，产出 CP-SAT 的 2 页版和 4 页版定稿。
3. 对比两版的信息完整度、阅读负担和生图难度。
4. 根据首篇样稿沉淀可复用的分镜与 Prompt 检查清单。
